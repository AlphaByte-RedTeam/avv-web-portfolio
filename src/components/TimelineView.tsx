...
import { Briefcase, Calendar, Folder, GraduationCap, Trophy } from 'lucide-react'
...
export const TimelineView = ({ items }: { items: TimelineItem[] }) => {
  return (
    <div className="relative max-w-3xl mx-auto py-10 px-4">
      {/* Central Line */}
      <div className="absolute left-12 md:left-1/2 top-0 bottom-0 w-px bg-border/60 -translate-x-1/2" />
            ...
            >
              ...
              >
                <div className="relative bg-secondary/20 hover:bg-secondary/30 transition-colors p-5 rounded-xl border border-border/50">
                  ...
                  {item.subtitle && (
                    <div className="text-sm text-primary font-medium mt-1">{item.subtitle}</div>
                  )}
                  ...
                  {item.tags && item.tags.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-3">
                      {item.tags.map((tag) => (
                        <span
                          key={tag}
                          className="text-[10px] px-1.5 py-0.5 rounded-sm bg-background/50 text-muted-foreground border border-border/50"
                        >
                          {tag}
                        </span>
                      ))}
