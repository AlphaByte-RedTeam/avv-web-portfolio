...
export const PromptsClient = ({ prompts }: { prompts: Prompt[] }) => {
  ...
  return (
    <div className="container py-10 max-w-5xl mx-auto space-y-8 px-4 md:px-6">
      ...
      >
        <h1 className="text-3xl font-bold tracking-tighter sm:text-4xl md:text-5xl">Prompts Library</h1>
              ...
              <Dialog>
                <DialogTrigger asChild>
                  <Card className="h-full flex flex-col hover:shadow-lg transition-all duration-300 cursor-pointer hover:border-primary/50 group">
                    <CardHeader>
                      <CardTitle className="line-clamp-1 group-hover:text-primary transition-colors">{prompt.title}</CardTitle>
                      {prompt.description && <CardDescription className="line-clamp-2">{prompt.description}</CardDescription>}
                    </CardHeader>
                    ...
                    <CardFooter className="flex gap-2 flex-wrap pt-0">
                       {prompt.tags?.map((t, i) => (
                         t.tag && <Badge key={i} variant="outline" className="text-[10px] px-1.5 py-0 h-5">{t.tag}</Badge>
                       ))}
                    </CardFooter>
                  </Card>
                </DialogTrigger>
